    ...
    <section className={embedded ? "h-full overflow-y-auto p-5 sm:p-8" : "page-container"}>
      <div className="mb-8">
        ...
        <p className="page-description">
          Get useful results while keeping control of your inbox.
        </p>
      </div>

      <div className="space-y-4">
        <article className="rounded-3xl border border-ui bg-white p-6">
          <h2 className="section-title">Ask naturally</h2>
          <p className="mt-2 text-sm leading-6 text-secondary">
            Tell Signal what outcome you want. Include a sender, time range, label, or topic when it
            matters.
          </p>
          <ul className="mt-4 grid gap-2 sm:grid-cols-2">
            {EXAMPLES.map((example) => (
              <li key={example} className="rounded-xl bg-surface px-4 py-3 text-sm">
                {example}
              </li>
            ))}
          </ul>
        ...
        <article className="rounded-3xl border border-ui bg-white p-6">
          ...
          <p className="mt-2 text-sm leading-6 text-secondary">
            Manage Gmail accounts and local preferences in Account. You can also remove all Signal
            conversation history without changing any Gmail messages.
          </p>
          <div className="mt-4 flex flex-wrap gap-3">
            {onOpenAccount ? (
              <button type="button" onClick={onOpenAccount} className="button-primary">
                Open Account
              </button>
            ) : (
